...
namespace O2S_MedicalRecord.GUI.FormCommon.TabCaiDat
{
    public partial class ucCauHinhHeThong : UserControl
    {
        ...
        private void HienThiThongBao(string tenThongBao)
        {
            try
            {
                O2S_MedicalRecord.Utilities.ThongBao.frmThongBao frmthongbao = new O2S_MedicalRecord.Utilities.ThongBao.frmThongBao(tenThongBao);
                frmthongbao.Show();
            }
            catch (Exception ex)
            {
        ...
        private void btnOptionOK_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (toolsoptionid != "")
                {
                    ...
                    {
                        HienThiThongBao(O2S_MedicalRecord.Base.ThongBaoLable.SUA_THANH_CONG);
                    }
                }
                else
                {
                    ...
                    if (condb.ExecuteNonQuery_HSBA(sqlupdate))
                    {
                        HienThiThongBao(O2S_MedicalRecord.Base.ThongBaoLable.THEM_MOI_THANH_CONG);
                    }
                }
        ...
        private void btnOptionDefault_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //    {
                //        HienThiThongBao(O2S_MedicalRecord.Base.ThongBaoLable.THAO_TAC_THANH_CONG);
                //        ucCauHinhHeThong_Load(null,null);
                //    }
                //    else
                //    {
                //        HienThiThongBao(O2S_MedicalRecord.Base.ThongBaoLable.CO_LOI_XAY_RA);
                //    }
                //}
            }
            catch (Exception ex)
