...
namespace MeO2_MedicalRecord.FormCommon
{
    public partial class frmConnectDB : Form
    {
        ...
        string en_licensekeynull = MeO2_MedicalRecord.Base.EncryptAndDecrypt.Encrypt("", true);
        public frmConnectDB()
        {
            InitializeComponent();
        }
        ...
        private void frmConnectDB_Load(object sender, EventArgs e)
        {
            ...
            this.txtDBHost_HSBA.Text = MeO2_MedicalRecord.Base.EncryptAndDecrypt.Decrypt(ConfigurationManager.AppSettings["ServerHost_HSBA"].ToString().Trim(), true);
            this.txtDBPort_HSBA.Text = MeO2_MedicalRecord.Base.EncryptAndDecrypt.Decrypt(ConfigurationManager.AppSettings["ServerPort_HSBA"].ToString().Trim(), true);
            this.txtDBUser_HSBA.Text = MeO2_MedicalRecord.Base.EncryptAndDecrypt.Decrypt(ConfigurationManager.AppSettings["Username_HSBA"].ToString().Trim(), true);
            this.txtDBPass_HSBA.Text = MeO2_MedicalRecord.Base.EncryptAndDecrypt.Decrypt(ConfigurationManager.AppSettings["Password_HSBA"].ToString().Trim(), true);
            this.txtDBName_HSBA.Text = MeO2_MedicalRecord.Base.EncryptAndDecrypt.Decrypt(ConfigurationManager.AppSettings["Database_HSBA"].ToString().Trim(), true);
        }
        ...
        private void btnDBKiemTra_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                conn_HSBA.Open();
                string sql_HSBA = "SELECT * FROM tbuser";
